...
RUN echo 'alias build="colcon build --symlink-install  --event-handlers console_direct+"' >> /etc/bash.bashrc && \
    echo 'source /colcon_ws/install/setup.bash; ros2 launch ros_as5048b ros_as5048b.launch.py i2c_device:=${I2C_DEVICE} i2c_address:=${I2C_ADDRESS} encoder_topic:=/encoder/at_${I2C_ADDRESS}/deg' >> /run.sh && chmod +x /run.sh && \
